...
mrdtm_proj <- terra::rast("data/processed/mrdtm_proj.tif")

### identify ridges on the landscape
mrtpi_proj <- spatialEco::tpi(mrdtm_proj, s = 25)
ridges <- mrtpi_proj >= 30
NAflag(ridges) <- FALSE
ridge_dist <- distance(ridges)
writeRaster(ridge_dist, "data/processed/ridge_dist.tif", overwrite = TRUE)
writeRaster(mrtpi_proj, "data/processed/mrtpi_proj.tif", overwrite = TRUE)
