...
akde_conts <- lapply(akde, as.sf, level.UD=0.95,level=0.95) %>%
  bind_rows() %>%
  mutate(Id_Number = as.numeric(str_split_i(name," ",1)),
         type = str_split_i(name," ",3)) %>%
  filter(type == "est")
saveRDS(akde_conts, here("output/akde_conts.rds"))
