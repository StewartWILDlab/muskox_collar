...
musk_collar_fix <- musk_collar %>%
  filter(Id_Number!=706|Date<xdate_706,
         Id_Number!=706|Date!="2009-01-29",
         Id_Number!=707|Date<xdate_707,
...
Lastly, we will attempt to remove the immediate effects of capture and handling. If we plot the first four weeks of each trajectory, we can see that there is a period where each muskox spends some time travelling before it ends up at an area that it stays for the rest of the winter.
...
deploy <- read_csv(here("data/raw/muskox_data/musk_collar_deployments.csv"), show_col_types = FALSE) %>%
  mutate(datetime = as.POSIXct(str_c(Date, Time, sep = " ")),
         ...
         date_filter = datetime + 12*7*24*60*60,
...
musk_deploy <- musk_collar_fix %>%
  ...
  group_by(Id_Number) %>%
  mutate(date_flag = ifelse(datetime == min(datetime),1,0))
...
pal <- leaflet::colorFactor(
  palette = c('blue', 'red'),
  domain = c(musk_deploy$date_flag)
)
...
post_depolyment_cutoff <- tibble(
  Id_Number = factor(c(706,707,708,709,710,7010,7011,7012,7013,7080)),
  post_deploy_date = c(
    as_datetime("2007-12-03 16:00:00"),
    as_datetime("2007-12-05 00:00:00"),
    as_datetime("2009-02-25 16:00:00"),
    as_datetime("2008-01-19 16:00:00"),
    as_datetime("2008-01-21 08:00:00"),
    as_datetime("2009-02-09 00:00:00"),
    as_datetime("2009-02-08 08:00:00"),
    as_datetime("2009-02-08 00:00:00"),
    as_datetime("2009-02-12 00:00:00"),
    as_datetime("2007-12-04 08:00:00")
  )
)
...
musk_collar_filt <- musk_collar_fix %>%
  right_join(sdl_dd2 %>% select(rowid, pTime, pDist, pSpeed),
            by = "rowid") %>%
  left_join(post_depolyment_cutoff) %>%
  ## filter out observations within the post deployment window identified from
  ## the trajectories
  filter(datetime > post_deploy_date) %>%
...
### create Movebank formatted file
musk_move <- musk_collar_filt %>%
  arrange(datetime, Id_Number) %>%
  mutate(event.id = 1:n(),
         visible = "true",
         timestamp = as.character(datetime),
         location.long = Longitude,
         location.lat = Latitude,
         sensor.type = "gps",
         individual.taxon.canonical.name = NA,
         tag.local.identifier = PTT,
         individual.local.identifier = Id_Number,
         study.name = "NWT Muskox") %>%
  select(event.id, visible, timestamp, , location.long, location.lat,
         sensor.type, individual.taxon.canonical.name, 
         tag.local.identifier, individual.local.identifier, study.name) %>%
  as_data_frame()

saveRDS(musk_move, here("data/processed/musk_move.rds"))
