...
The winter home ranges appear to have shallower snow depth compared to the area surrounding the ranges. In fact, only two ranges out of the 11 have deeper snow and these were during years with comparatively less snow. Though the average difference is only 2.3 cm, the snow depth data are quite coarse (3 km resolution), so even the smallest difference is interesting.

## Calving Season Model

```{r}
#| label: musk_snow_calv
#| dependson: "musk_snow"
...
snow_func2 <- function(id){
  ...
  zones <- rasterize(points %>%
                        terra::vect(),
                    sm_crop, field = "group", touches = TRUE)
  rowids <- which(month(daterange)==4&year(daterange)==year)
  zonal(sm_crop[[rowids]], zones, "mean", wide = FALSE) %>%
    group_by(group) %>%
    mutate(date = daterange[rowids], 
...
musk_snow_calv <- musk_seascont %>%
  filter(season == "Calving", range == "home") %>%
  pull(range_id) %>%
  map(function(.x){snow_func2(.x)}) %>%
  list_rbind() 
```

```{r}

musk_snow_calv2 <- musk_snow_calv %>%
  group_by(range_id, group) %>%
  summarise(snow_depth = mean(value)) %>%
  left_join(musk_seascont %>%
              filter(range == "home") %>%
...
wint_mod2 <- nlme::lme(snow_depth~group, random=~1|Id_Number/year_min, 
                 data = musk_snow_calv2)
summary(wint_mod2)
```

Once again, the patterns with snow are not as apparent during the calving season. While there is a slight decrease in snow depth for the home range compared to the area surrounding the home range, the difference is not as large or significant. This may be partially due to the fact that the calving range only covers a portion of the full winter range. There might be some other landscape variable that is a better determinant of calving areas (perhaps identified through resource selection analysis).
