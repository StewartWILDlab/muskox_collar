...
# hr_landcover <- readRDS(here("output/hr_landcover.rds"))
...
# musk_tpi <- readRDS(here("output/musk_tpi.rds"))
```

```{r}
#| label: canopy_pseudo
#| dependson: "tpi_pseudo"
#| echo: FALSE

### Removed, could not find any reason to suggest canopy cover was signifcantly selected in winter
# prcC_2020_crop <- rast("data/processed/prcC_2020_crop.tif")
# 
# musk_canopy <- musk_pseudohr %>%
#   pull(range_id) %>%
#   map(function(.x){tpi_func(.x, prcC_2020_crop)}) %>%
#   list_rbind()
# saveRDS(musk_canopy, here("output/musk_canopy.rds"))
...
wint_effects <- wint_pseudo_brm %>%
  tidybayes::spread_draws(`Intercept`, `b_scalevalue`, `b_scalefor_per`, 
                          `sd_Id_Number__Intercept`) %>%
  tidybayes::summarise_draws(~quantile(.x, probs = c(0.025,0.5,0.975))) %>%
  mutate(Parameter = c("Intercept", "TPI", "Proportion \ngrassland/shrubland", "sd Muskox ID"),
         Effect = c("Fixed", "Fixed", "Fixed", "Random"),
         Season = "Winter")
