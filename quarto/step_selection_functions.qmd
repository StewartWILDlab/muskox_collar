...
musk_collar_season <- musk_collar_filt %>%
  ...
  mutate(season = season_delineation(datetime, snow_depth, 0.01),
         # season = ifelse(month >= 4 & month <= 7 & season == "winter", "calving", season),
...
issa_data <- musk_collar_season %>%
  ...
  sf::st_drop_geometry() %>%
  ### remove 709 because it follows the same path as 710
  filter(Id_Number != 709)
...
issa_bursts_summer <- issa_track_nest %>%
  ...
  mutate(
    ...
    rand_steps = map(steps, function(x){
      set.seed(5)
      x %>% random_steps(n_control = 20)
      })
  )

issa_bursts_winter <- issa_track_nest %>%
  filter(season %in% c("winter", "calving")) %>%
  mutate(
    data_resamp = map(data, function(x)
      x %>% track_resample(rate = hours(8), tolerance = hours(1))
      ),
...
issa_data_summer_comb <- list_rbind(issa_data_summer$data_covs) %>%
  ...
  mutate(log_water_dist = log(layer/30+1),
         ...
         avg_max_temp_poly = poly(avg_max_temp,2),
         ...
         dummy_value = 1,
         ### creating duplicate fire year vars for the fire distance 
         ### interactions to make it easier to calculate RSS later
         fire_10_dup = fire_10,
         fire_20_dup = fire_20,
         fire_30_dup = fire_30,
         fire_40_dup = fire_40,
  ...
  mutate(Wetland = Water + Wetland,
         Temperate_or_sub_polar_grassland = Temperate_or_sub_polar_grassland + 
           Sub_polar_or_polar_grassland_lichen_moss,
         Temperate_or_sub_polar_needleleaf_forest = Temperate_or_sub_polar_needleleaf_forest + 
           Mixed_forest)
...
issa_data_winter_comb <- list_rbind(issa_data_winter$data_covs) %>%
  ...
  mutate(log_water_dist = log(layer/100+1),
         log_ridge_dist = log(ridge_dist/30+1),
         log_fire_dist = log(fire_dist/30+1),
         avg_min_temp_poly = poly(avg_min_temp,2),
         ...
         fire_10_20_30 = ifelse(fire_year<=30,1,0),
         fire_30 = ifelse(fire_year>20&fire_year<=30,1,0),
         fire_40 = ifelse(fire_year>30&fire_year<=40,1,0),
         dtm_diff = mrdtm_end - mrdtm_start,
         calving = ifelse(month(t1_)>=4&month(t1_)<=7, 1, 0),
         dummy_value = 1,
         focal_modal = str_replace_all(focal_modal, " ", "_"),
         focal_modal = str_replace_all(focal_modal, "-", "_")) %>%
  pivot_wider(names_from = focal_modal, values_from = dummy_value, values_fill = 0) %>%
...
m0_summer_glmmfit <- glmmTMB(case_ ~ -1 + scale(log_sl)*scale(cos_ta) + 
                               ...
                               scale(log_fire_dist) +
                               scale(log_fire_dist):fire_10_dup +
                               scale(log_fire_dist):fire_20_dup +
                               scale(log_fire_dist):fire_30_dup +
                               scale(log_fire_dist):fire_40_dup +
                               scale(tpi) +
                               water +
                               # Wetland + 
                               fire_10_low +
                               ...
                               (0 + scale(log_fire_dist):fire_10_dup|id) +
                               (0 + scale(log_fire_dist):fire_20_dup|id) +
                               (0 + scale(log_fire_dist):fire_30_dup|id) +
                               (0 + scale(log_fire_dist):fire_40_dup|id) +
                               (0 + water|id) +
                               # (0 + Wetland|id) +
                               (0 + scale(mrdtm)|id) +
                               (0 + fire_10_low|id) +
                               ...
                               (0 + fire_40|id) +
                               # (0 + fire_50|id) +
                               (0 + scale(log_sl):scale(avg_max_temp_1)|id) +
                               (0 + scale(log_sl):scale(avg_max_temp_2)|id), 
                             family = poisson,
                             data = issa_data_summer_comb,
                             map=list(theta=factor(c(NA,1:23))),
                             start=list(theta=c(log(1e3),rep(0,23))))
...
m0_summer_glmmfit_lc <- glmmTMB(case_ ~ -1 + scale(log_sl)*scale(cos_ta) + 
                               ...
                               (0 + Wetland|id) +
                               (0 + Barren_Lands|id) +
                               (0 + scale(avg_max_temp_1):scale(log_sl)|id)+
                               (0 + scale(avg_max_temp_2):scale(log_sl)|id), 
                             family = poisson,
                             data = issa_data_summer_comb,
                             map=list(theta=factor(c(NA,1:16))),
                             start=list(theta=c(log(1e3),rep(0,16))))
summary(m0_summer_glmmfit_lc)

saveRDS(m0_summer_glmmfit_lc, here("output/m0_summer_glmmfit_lc.rds"))

m0_summer_glmmfit_null <- glmmTMB(case_ ~ -1 + scale(log_sl)*scale(cos_ta) + 
                                    scale(sl_) +
                               scale(mrdtm) +
                               scale(tpi) +
                               scale(avg_max_temp_1):scale(log_sl) +
...
summary(m0_summer_glmmfit_null)


```

```{r}
#| label: summer_models_web
#| dependson: "combined_data"

m0_summer_glmmfit <- readRDS(here("output/m0_summer_glmmfit.rds"))
summary(m0_summer_glmmfit)
  
# m0_summer_glmmfit_lc <- readRDS(here("output/m0_summer_glmmfit_lc.rds"))
# summary(m0_summer_glmmfit_lc)
...
m0_winter_glmmfit <- glmmTMB(case_ ~ -1 + scale(log_sl)*scale(cos_ta) + 
                               ...
                               fire_40 +
                               scale(log_fire_dist) +
                               # fire_10_20_30:scale(log_fire_dist) +
                               # fire_40:scale(log_fire_dist) +
                               calving:scale(log_sl) +
                               ...
                               (0 + scale(avg_min_temp_1):scale(log_sl)|id) +
                               (0 + scale(avg_min_temp_2):scale(log_sl)|id) +
                               (0 + fire_10_20_30|id) +
                               (0 + fire_40|id) +
                               # (0 + fire_10_20_30:scale(log_fire_dist)|id) +
                               # (0 + fire_40:scale(log_fire_dist)|id) +
                               (0 + calving:scale(log_sl)|id), 
                             family = poisson,
                             data = issa_data_winter_comb,
                             map=list(theta=factor(c(NA,1:11))),
                             start=list(theta=c(log(1e3),rep(0,11))))
...
saveRDS(m0_winter_glmmfit_lc, "output/m0_winter_glmmfit_lc.rds")

m0_winter_glmmfit_null <- glmmTMB(case_ ~ -1 + scale(log_sl)*scale(cos_ta) +
                                    scale(sl_) +
                                  scale(mrdtm) +
                                  scale(tpi) +
                                  calving:scale(log_sl) +
                                ...
                                map=list(theta=factor(c(NA,1:14))),
                                start=list(theta=c(log(1e3),rep(0,14))))
summary(m0_winter_glmmfit_null)

```

```{r}
#| label: winter_models_web
#| dependson: "combined_data"

m0_winter_glmmfit <- readRDS(here("output/m0_winter_glmmfit.rds"))
summary(m0_winter_glmmfit)

m0_winter_glmmfit_lc <- readRDS(here("output/m0_winter_glmmfit_lc.rds"))
summary(m0_winter_glmmfit_lc)
...
#| dependson: "winter_models"
#| eval: FALSE

set.seed(25)
test_collars <- sample(unique(issa_data_summer_comb$id),2)


### Summer
issa_data_summer_comb_sub <- issa_data_summer_comb %>%
  filter(!(id %in% test_collars))

m0_summer_glmmfit_sub <- glmmTMB(case_ ~ -1 + scale(log_sl)*scale(cos_ta) + 
                                   scale(sl_) +
                               scale(mrdtm) +
                               scale(log_water_dist) +
                               scale(log_fire_dist) +
                               ...
                               (0 + scale(log_sl):scale(avg_max_temp_2)|id), 
                             family = poisson,
                             data = issa_data_summer_comb_sub,
                             map=list(theta=factor(c(NA,1:23))),
                             start=list(theta=c(log(1e3),rep(0,23))))
summary(m0_summer_glmmfit_sub)

preds_summ <- generate_uhc_predictions(m0_summer_glmmfit_sub,
                                  issa_data_summer_comb,
                                  test_collars,
                                  nsim = 100)
saveRDS(preds_summ, here("output/uhc_preds_summ.rds"))

ggplot(data = preds_summ %>% filter(data == "simulation"), aes(x = mrdtm)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_summ %>% filter(data == "observed"), colour = "red")
ggplot(data = preds_summ %>% filter(data == "simulation"), aes(x = tpi)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_summ %>% filter(data == "observed"), colour = "red")
ggplot(data = preds_summ %>% filter(data == "simulation"), aes(x = log_fire_dist)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_summ %>% filter(data == "observed"), colour = "red")
ggplot(data = preds_summ %>% filter(data == "simulation"), aes(x = log_water_dist)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_summ %>% filter(data == "observed"), colour = "red")
ggplot(data = preds_summ %>% filter(data == "simulation"), aes(x = water)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_summ %>% filter(data == "observed"), colour = "red")
ggplot(data = preds_summ %>% filter(data == "simulation"), aes(x = log_sl)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_summ %>% filter(data == "observed"), colour = "red")
ggplot(data = preds_summ %>% filter(data == "simulation"), aes(x = cos_ta)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_summ %>% filter(data == "observed"), colour = "red")

preds_summ %>%
  pivot_longer(starts_with("fire")&-ends_with("dup"), 
               names_to = "fire", values_to = "value") %>%
  group_by(data, sim, fire) %>%
  summarise(count = sum(value)) %>%
  group_by(data, sim) %>%
  mutate(prop = count/sum(count)) %>%
  ungroup() %>%
  arrange(desc(data)) %>%
  ggplot(aes(x = fire, y = prop,
             colour = data, group = sim, alpha = data)) +
  geom_point() +
  scale_colour_manual(values = c("red", "grey")) +
  scale_alpha_manual(values = c(1, 0.5))

### Winter
issa_data_winter_comb_sub <- issa_data_winter_comb %>%
  filter(!(id %in% test_collars))

m0_winter_glmmfit_lc_sub <- glmmTMB(case_ ~ -1 + scale(log_sl)*scale(cos_ta) +
                                  scale(sl_) +    
                                  scale(mrdtm) +
                                  scale(tpi) +
                                  Wetland +
                                  ...
                                  (0 + Temperate_or_sub_polar_grassland|id) +
                                  (0 + Barren_Lands|id) +
                                  (0 + scale(log_sl):calving|id) +
                                  (0 + scale(log_sl):scale(avg_min_temp_1)|id) +
                                  (0 + scale(log_sl):scale(avg_min_temp_2)|id) +
                                  (0 + scale(log_sl):scale(snow_depth)|id) , 
                                family = poisson,
                                data = issa_data_winter_comb_sub,
                                map=list(theta=factor(c(NA,1:15))),
                                start=list(theta=c(log(1e3),rep(0,15))))
summary(m0_winter_glmmfit_lc_sub)

preds_wint <- generate_uhc_predictions(m0_winter_glmmfit_lc_sub,
                                  issa_data_winter_comb,
                                  test_collars,
                                  nsim = 100)
saveRDS(preds_wint, here("output/uhc_preds_wint.rds"))

ggplot(data = preds_wint %>% filter(data == "simulation"), aes(x = mrdtm)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_wint %>% filter(data == "observed"), colour = "red")
ggplot(data = preds_wint %>% filter(data == "simulation"), aes(x = tpi)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_wint %>% filter(data == "observed"), colour = "red")
ggplot(data = preds_wint %>% filter(data == "simulation"), aes(x = log_sl)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_wint %>% filter(data == "observed"), colour = "red")
ggplot(data = preds_wint %>% filter(data == "simulation"), aes(x = cos_ta)) +
  geom_density(aes(group = sim), colour = "grey", alpha = 0.5) +
  geom_density(data = preds_wint %>% filter(data == "observed"), colour = "red")

preds_wint %>%
  pivot_longer(c(Wetland, Sub_polar_taiga_needleleaf_forest,
                 Temperate_or_sub_polar_shrubland,
                 Temperate_or_sub_polar_grassland,
                 Barren_Lands), 
               names_to = "lc", values_to = "value") %>%
  group_by(data, sim, lc) %>%
  summarise(count = sum(value)) %>%
  group_by(data, sim) %>%
  mutate(prop = count/sum(count)) %>%
  ungroup() %>%
  arrange(desc(data)) %>%
  ggplot(aes(x = lc, y = prop,
             colour = data, group = sim, alpha = data)) +
  geom_point() +
  scale_colour_manual(values = c("red", "grey")) +
  scale_alpha_manual(values = c(1, 0.5))
