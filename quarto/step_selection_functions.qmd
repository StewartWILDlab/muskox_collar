...
issa_data_winter_comb %>%
  filter(case_ == TRUE) %>%
  ggplot(aes(x = avg_min_temp, y = log_sl)) +
  geom_point() +
  geom_smooth()

issa_data_summer_comb %>%
  filter(case_ == TRUE) %>%
  ggplot(aes(x = fire_year, y = tpi)) +
  geom_point() +
  geom_smooth()
...
m0_summer_glmmfit <- glmmTMB(case_ ~ -1 + scale(log_sl)*scale(cos_ta) + 
                             ...
                             start=list(theta=c(log(1e3),rep(0,18))))
